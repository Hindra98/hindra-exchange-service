...
export const AlertSuccess = ({ children, className = "", id }: AlertProps) => (
  <motion.div
    ...
    className={
      className +
      " text-lime-600 bg-card *:data-[slot=alert-description]:text-lime-600/90 relative w-full rounded-lg border px-4 py-3 text-sm grid has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] has-[>svg]:gap-x-3 gap-y-0.5 items-start [&>svg]:size-4 [&>svg]:translate-y-0.5 [&>svg]:text-current"
    }
  >
    {children}
  </motion.div>
