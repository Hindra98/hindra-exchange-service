...
export class LocalizationProvider {
  ...
  public static readonly parentCulture =
  ...
  public load(culture: string = ""): Promise<void> {
    if (
      LocalizationProvider.parentCulture === culture &&
      !IsLanguageTranslationAvailable(culture)
    ) return Promise.resolve();
    const fileName: string = this.name; let fileKey: string;
    if (culture !== undefined) fileKey = `${fileName}.${culture}`;
    else fileKey = `${fileName}.${getBrowserLanguage()}`;
  ...
  public get loaded() {
    return (
      LocalizationProvider.loadedResources.has(
        `${this.name}.${LocalizationProvider.culture}`
      ) &&
      !(
        LocalizationProvider.loadedResources.get(
          `${this.name}.${LocalizationProvider.culture}`
        ) instanceof Promise
      )
    );
  ...
  public get(key: string) {
    ...
    if (
      ...
      IsLanguageTranslationAvailable(LocalizationProvider.parentCulture)
    ) {
      foundValue = (
        LocalizationProvider.loadedResources.get(
          `${this.name}.${LocalizationProvider.culture}`
